  ...
  , m_stateMachineWatcher(new StateMachineWatcher(probe, this))
{
  ...
  connect(m_ui->stateMachinesView, SIGNAL(clicked(QModelIndex)), SLOT(handleMachineClicked(QModelIndex)));

  connect(m_ui->singleStateMachineView, SIGNAL(clicked(QModelIndex)), SLOT(handleStateClicked(QModelIndex)));
...
void StateMachineViewer::addState(QAbstractState* state)
{
  ...
  if (parentState && parentGraphId) {
    ...
    Q_ASSERT(m_graph->addEdge(m_stateNodeIdMap[parentState], nodeId,
        QString("%1 -> %2").arg(Util::displayString(parentState)).arg(Util::displayString(state))));
  } else {
...
void StateMachineViewer::addTransition(QAbstractTransition* transition)
{
  ...
  EdgeId id = m_graph->addEdge(sourceStateId, targetStateId,
      Util::displayString(transition));
  Q_ASSERT(id);
