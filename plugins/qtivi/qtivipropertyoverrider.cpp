...
QVariant QtIviPropertyOverrider::iviValue() const
{
    if (isOverride()) {
        QIviPropertyPrivate *const d = QIviPropertyPrivate::get(m_prop);
        return iviValue(m_overrideValue, d->m_type);
    }

    return m_prop->value();
}

QVariant QtIviPropertyOverrider::iviValue(const QVariant &value, int userType)
{
    QVariant var = value;

    //Try to convert the m_overrideValue, if successfully, use the converted value
    QVariant temp(var);
    if (temp.convert(userType))
        var = temp;
    ...
    if (mo) {
        ...
        if (mEnum.isValid()) {
            ...
            if (!mEnum.isFlag() && !mEnum.valueToKey(var.toInt())) {
                qWarning("Enum value out of range");
                return QVariant();
            }
        }
    }
    ...
    if (!isEnumOrFlag && var.typeName() != QVariant::typeToName(userType)) {
        qWarning("Expected: %s but got %s", QVariant::typeToName(userType), QVariant::typeToName(value.userType()));
        return QVariant();
    }

    return QVariant(userType, var.data());
}
